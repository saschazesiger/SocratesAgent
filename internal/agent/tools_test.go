...
// The interactive path is the whole point, so it must stay open: a skill named
// in terminal_open is started in a real terminal, and what it prints on the
// way up is what the model gets to read.
func TestTerminalOpenStartsASkill(t *testing.T) {
	...
	router := &mockRouter{responses: []string{
		sseToolCall("terminal_open", `{"skill":"claude","name":"claude test"}`),
	...
	chat := &store.Chat{ID: "chat-open-skill"}
