...
type OpenRouterSettings struct {
	...
	// StatusModel reads a screen and says what it means. Nothing in the app
	// validates the id: there is no catalogue endpoint left, so it is free
	// text and a model that does not exist is only found out when OpenRouter
	// refuses it.
...
func (s *Settings) Normalize() {
	...
	// The model the Status button uses. An empty field is a form that was
	// never filled in, not a request for no model: without one the button
	// would answer "pick a model" on a fresh install, which is a setup step
	// nobody asked for.
