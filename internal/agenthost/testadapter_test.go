...
// adapters to WP2/3/4. So this package ships one of its own - in-process,
// scripted by an environment variable in the same shape as the fakes' script,
// and registered under the id "test".
//
...
// testScriptEnv is read from the Spec's Env - so each host can be scripted
// separately - and falls back to the process environment.
const testScriptEnv = "SOCRATES_TEST_SCRIPT"
...
// step is one instruction of the script.
type step struct {
	Do      string `json:"do"`
...
func (a *testAdapter) Start(ctx context.Context, spec harness.Spec) error {
	...
	for _, kv := range spec.Env {
		if v, ok := strings.CutPrefix(kv, testScriptEnv+"="); ok {
			raw = v
		}
	}
	if raw == "" {
		raw = os.Getenv(testScriptEnv)
	}
	if raw != "" {
