...
func TestSendAfterACrashRestartsTheAdapter(t *testing.T) {
	...
	// it got a session_id, a turn_started and a turn_finished of its own,
	// which is what proves a second adapter was really started and really
	// answered.
	kinds := map[string]int{}
	for _, ev := range events {
		kinds[ev.Kind]++
	}
	if kinds[harness.KindSessionID] != 1 {
		t.Errorf("the restarted adapter did not report its resumed session: %#v", kinds)
	}
...
func TestReplayBelowTheTrimPointIsRefused(t *testing.T) {
	...
	// A fresh subscription - from_seq 0 - is always answerable.
	if err := j.replay(0, func(harness.Event) bool { return true }); err != nil {
		t.Fatalf("a subscription from the start was refused: %v", err)
	}
}
